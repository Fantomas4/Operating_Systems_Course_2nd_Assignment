...
{ 
	...
	static ArrayList<Integer> worstFit(int sizeOfBlocks[], int sizeOfProcesses[])
	{
		...
		for (int i = 0; i < sizeOfProcesses.length; i++) {
			...
			// find the block that has the maximum size
			int max_value = sizeOfBlocks[0];
			int max_pos = 0;

			for (int k = 0; k < sizeOfBlocks.length; k++) {
				if (sizeOfBlocks[k] > max_value) {
					max_value = sizeOfBlocks[k];
					max_pos = k;
				}
			}

			if (sizeOfProcesses[i] <= max_value) {
				result.add(max_pos);
				sizeOfBlocks[max_pos] -= sizeOfProcesses[i];
			} else {
				// memory allocation for selected process not possible
				result.add(-255);
			}
